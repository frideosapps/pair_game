...
///
///
/// It's the simplest object derived from the [StreamObjectBase] class.
///
...
///
///
/// HistoryObject extends the [MemoryObject] class, adding a collection and a stream.
///
...
class HistoryObject<T> extends MemoryValue<T> {
  final _historyStream = StreamedCollection<T>();
  ...
  ///
  saveValue() {
    _historyStream.addElement(_value);
  }
...
class TimerObject extends StreamedValueBase<int> {
  ...
  ///
  startPeriodic(Duration interval, callback) {
    if (!isTimerActive) {
